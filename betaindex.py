...
def get_args():
    ...
    parser.add_argument(
        '-b', '--bottom', type=int, help='Choose top or bottom ranked stocks: 0 - top n stocks, 1 - bottom n stocks (Default 0).', required=False, default=1)    
    parser.add_argument(
    ...
    parser.add_argument(
        '-s', '--solver', type=int, help='Choose solver: 0 - least squares, 1 - least absolute deviation (Default 0).', required=False, default=0)
    parser.add_argument(
    ...
    parser.add_argument(
        '-pr', '--prnorm', type=int, help='Percentile normalization of stock prices: 1 - Yes, 0 - No (Default 1).', required=False, default=1)
    parser.add_argument(
    ...
    return filename, pastn, rate, topn, initcapital, order, solver, returnrate, weight, tradecost, spread, verbose, msefile, startn, bottom, prnorm


def main():
    filename, pastn, rate, topn, initcapital, order, solver, returnrate, weight, tradecost, spread, verbose, msefile, startn, bottom, prnorm = get_args()
    ...
    for k in range(pastn, n, rate):
        ...
        if (solver == 1) & (msefile is None):
            mse = get_lad_mse(X[0:pastn,iValid],prnorm)
            MSE[count,iValid] = mse
            mse = MSE[count,:]
        elif msefile is None:    
            #mse = get_ls_mse(X[0:pastn,iValid],prnorm)
            #mse = get_sd_mse(X[0:pastn,iValid])
            mse = get_beta(X[0:pastn,iValid])
            MSE[count,iValid] = mse
            mse = MSE[count,:]
        else:
            mse = MSE[count,:]
...
def get_ls_mse(X,prnorm):
    ...
    A = np.zeros([m*(m-1)/2, m], dtype=np.float32)
    b = np.zeros([m*(m-1)/2, ], dtype=np.float32)
    k = 0
    for i in range(0,m):
        for j in range(0,i):
            A[k,i] = 1
            A[k,j] = 1
            b[k] = metrics.mean_squared_error(X[:,i], X[:,j])
            k = k + 1
...
def get_sd_mse(X):
    ...
    X = X/X[0,:]
    M = sp.spatial.distance.squareform((1/m)*sp.spatial.distance.pdist(np.transpose(X),'sqeuclidean'))
    EM = np.exp(M)
